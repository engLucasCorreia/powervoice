...
> **Status: early preview (v0.4.0).** PowerVoice is developed and tested on **Linux**. Windows and
> macOS installers are built by CI and have never been run by anyone — treat them as unverified
> (see [Download](#download)). Expect rough edges, and please report what you find.
...
**Windows and macOS:** CI builds an installer for each on every tagged release (`.msi` and `.exe`
on Windows, `.dmg` on macOS) and attaches it above. **Treat these as unverified: they compile and
package on CI, but nobody has installed or run them.** They are also unsigned and unnotarized, so
Windows SmartScreen and macOS Gatekeeper will warn, and the macOS build is Apple Silicon only. If
you try one, please report what happens; otherwise [build from source](docs/building.md).
