...
mod tests {
    ...
    /// H-59: `VXPK` was the one binary frame of ADR-003 §2 with no Rust-generated golden fixture —
    /// its Rust and TS tests each built their own bytes from the spec table, so a drift on one
    /// side could never be caught by the other. Writes the golden frames (one bucket frame, one
    /// `RAW` frame) for `ui/src/lib/waveform/vxpkFixture.test.ts` (ADR-003 §4: generated by
    /// `just gen-types`, checked by `just check-types`).
    #[test]
    fn export_bindings_vxpk_fixture() {
        // Bucket values are a pure function of the index, so the TS side recomputes them:
        // bucket i = (-1 - i/8, 1 + i/8) rounded through f32.
        let buckets: Vec<(f32, f32)> = (0..4)
            .map(|i| {
                let d = 1.0 + i as f32 / 8.0;
                (-d, d)
            })
            .collect();
        let bucket_frame = VxpkHeader {
            request_id: 9,
            audio_rev: 42,
            start_sample: 4_294_967_296, // > u32: proves the u64 split
            samples_per_bucket: 1024,
            sample_rate_hz: 48_000,
            partial: true,
        };
        // `samples_per_bucket == 1` is the RAW convention: only each pair's first value is written.
        let raw_frame = VxpkHeader {
            request_id: 10,
            audio_rev: 42,
            start_sample: 0,
        ...
        let entry = |h: &VxpkHeader| {
            let hex: String = encode_vxpk(h, &buckets)
                .iter()
                .map(|b| format!("{b:02x}"))
                .collect();
            format!(
                "  {{\n    hex: \"{hex}\",\n    fields: {{\n      requestId: {},\n      \
                 ...
                 samplesPerBucket: {},\n      count: {},\n      sampleRateHz: {},\n    }},\n  }},\n",
                h.request_id,
                h.samples_per_bucket == 1,
                h.partial,
                h.audio_rev,
                h.start_sample,
                h.samples_per_bucket,
                buckets.len(),
                h.sample_rate_hz,
            )
        };
        let ts = format!(
             ...
             * Both frames carry the same {n} buckets: bucket `i` is `(-(1 + i / 8), 1 + i / 8)`; the `RAW`\n \
        ...
        std::fs::write(dir.join("vxpk_fixture.ts"), ts).unwrap();
    }
}
