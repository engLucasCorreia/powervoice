...
pub fn rack_ipc_error(err: RackApiError) -> IpcError {
    match err {
        ...
        RackApiError::NoExtension => {
            IpcError::new(IpcErrorCode::NotFound, "error.rack_no_extension")
        }
...
mod tests {
    ...
    /// Writes the golden `VXTC` frames for the TS decoder test (H-77, SPEC-016 §4.12 / AC-23:
    /// one without `HAS_FALLING` and one with it, including a muted (−∞) level). Generated by
    /// the `export_bindings` run of `just gen-types`, checked by `just check-types`.
    #[test]
    fn export_bindings_vxtc_fixture() {
        use vox_engine::{TransferCurveHandle, TransferCurvePoints};
        use vox_rack::ParamId;

        // Values are exactly representable as f32, so the TS test compares them without a
        // tolerance.
        let plain = TransferCurvePoints {
            in_dbfs: vec![-80.0, -40.0, 0.0],
            rising_db: vec![-80.0, -40.0, -6.5],
            falling_db: None,
            components_db: vec![vec![0.0, 0.0, -6.5]],
            handles: vec![TransferCurveHandle {
                component: 0,
                param: ParamId(31),
                x_dbfs: -16.5,
                offset_db: 3.5,
        ...
        let gated = TransferCurvePoints {
            in_dbfs: vec![-80.0, -40.0, 0.0],
            rising_db: vec![f64::NEG_INFINITY, -40.0, -6.5],
            falling_db: Some(vec![f64::NEG_INFINITY, f64::NEG_INFINITY, -6.5]),
            components_db: vec![vec![f64::NEG_INFINITY, 0.0, 0.0], vec![0.0, 0.0, -6.5]],
            handles: vec![
                TransferCurveHandle {
                    component: 0,
                    param: ParamId(11),
        ...
        let hex = |p: &TransferCurvePoints, seq: u32| -> String {
            p.encode(seq).iter().map(|b| format!("{b:02x}")).collect()
        };
        let ts = format!(
            ...
            plain_hex = hex(&plain, 7),
            gated_hex = hex(&gated, 8),
        );
        let dir = std::env::var_os("TS_RS_EXPORT_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|| PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("bindings"));
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(dir.join("vxtc_fixture.ts"), ts).unwrap();
    }
}
