...
| H-33 | Small fixes (H-22): `sanitize_preset_name` must never yield a leading `.` (hidden by `list_json_names` as an atomic temp file), de-flake `WelcomeOffer.test.ts` (full-suite-only failure) | S | in progress |
| H-34 | Plugin polish (T-806): de-flake `sandbox/tests/teardown.rs::nothing_outlives_its_owner` (fd baseline after warm-up release), Linux Install Module wording for picking a file inside a `.vst3` bundle, rename scan cache to `plugin-scan.json` with migration | S | todo |
