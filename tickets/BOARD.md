...
| H-105 | Documentation refresh #2: everything since H-78 — Explain My Voice, octave-guarded pitch, the changed advice, loop/selection/import behaviour, export reliability, the AppImage bundling rules | S | in progress |
| H-106 | OWNER REQUEST: make the tours teach the app — noise and punch have one step each today, and a tour for Explain My Voice is missing | S | in progress |
| H-107 | OWNER REQUEST: a Help section inside the app (searchable, offline) covering how to use it and how to install third-party plugins — today Help has only About, Tours and Shortcuts | S | in progress |
