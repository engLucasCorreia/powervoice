...
| H-26 | Design follow-ups (H-25): Windows dialog button order (primary first), shared menu/popover component for all menus, migrate `lucide-svelte` (deprecated) → `@lucide/svelte`, Unicode minus in analyzer dB labels, owner-verified screens with a file open and while recording | S | todo |
...
| T-803 | CLAP adapter (+ enumerate) | W3 | O | T-802 | in progress |
