...
| H-54 | The last failing performance row: `frame_spectral_canvas2d_2126x850_p95_ms` 21.2 ms vs SPEC-007 AC-10's ≤ 16.7 ms — the Canvas2D (software) split view at a large window during a continuous zoom+scroll sweep. Either optimise further (the WebGL2 path passes everywhere) or amend AC-10 to scope the 60 fps target per renderer/window size, with the owner's 2126×850 window in mind | S | in progress |
...
| H-70 | Save as a real job (H-60, SPEC-005 §4.10): `JobKind::Save` with progress/cancel and `error.save.in_progress`, plus the §2.7 step-1 pre-write free-space estimate using H-11's `FreeSpaceProvider` | S | in progress |
...
| H-67 | Notice actions (H-59, SPEC-002 AC-7): notices have no action affordance anywhere — add `Notice.action` + a button in Banner/Toast, first user being "Go to first" jumping to the first dropout marker | S | in progress |
