...
- Binary skeleton `crates/cli` → bin name `voxedit-cli` using `clap` derive; `--version` works; subcommands `gen`, `analyze`, `render`, `bench` exist and print "not implemented yet".
...
- `justfile` recipes:
  ...
  - `check-cross`: `cargo check --workspace --exclude voxedit-app --target x86_64-pc-windows-gnu` (the exclude must not fail when the crate doesn't exist yet — just list the non-Tauri crates explicitly with `-p`).
...
- [ ] `cargo run -q -p cli -- --version` (or the crate's package name) prints `voxedit-cli 0.1.0`.
